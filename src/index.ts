...
function readTOMLConfig(options: options): {
  ...
  // To Add Custom Gen Options (Step 1/2)
  const loadedCustomGenerationOptions = fs.readJsonSync(
    path.resolve(process.cwd(), '.tomg-gen-config')
  );
  ...
  if (loadedGenerationOptions) {
    ...
    // To Add Custom Gen Options (Step 2/2)
    Object.keys(loadedCustomGenerationOptions).forEach((key) => {
      if (
        Object.prototype.hasOwnProperty.call(options.generationOptions, key)
      ) {
        options.generationOptions[key] = loadedCustomGenerationOptions[key];
      } else {
        console.error(`Unknown generation option ${key}.`);
        hasUnknownProperties = true;
      }
